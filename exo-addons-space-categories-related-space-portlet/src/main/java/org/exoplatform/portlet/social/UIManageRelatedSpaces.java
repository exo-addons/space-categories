 ...
 */
@ComponentConfig(
        template = "app:/groovy/social/portlet/relatedSpacesPortlet/UIManageRelatedSpaces.gtmpl",
        events = {
...
public class UIManageRelatedSpaces extends UIContainer {
    ...
    public List<Space> getUnRelatedSpacesList() throws Exception {
        updateRelatedUnrelatedSpaces();
        setEnableLoadNext((this.unRelatedSpacesList.size() >= SPACES_PER_PAGE)
                && (this.unRelatedSpacesList.size() < getSpacesNum()));
    ...
    public void loadNext() throws Exception {
        ...
        if (currentLoadIndex <= getSpacesNum()) {
            this.unRelatedSpacesList.addAll(new ArrayList<Space>(Arrays.asList(getSpacesListAccess()
    ...
    /**
     * This action is triggered when user click on RelateSpace <br /> The leaving space will remove
    ...
    /**
     * This action is triggered when user click on UnRelateSpace <br /> The leaving space will remove
     * that user in the space. <br /> If that user is the only leader -> can't not leave that space
     * <br />
     */
