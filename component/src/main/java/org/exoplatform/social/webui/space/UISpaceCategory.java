 ...
 * minh.dang@exoplatform.com Oct 18, 2006 2:12:26 PM
 */
@ComponentConfigs({ @ComponentConfig(template = "war:groovy/social/webui/space/UISpaceCategory.gtmpl"),
    @ComponentConfig(type = UIOneTaxonomySelector.class, id = "uiOneTaxonomySelector", template = "classpath:groovy/ecm/webui/UIOneTaxonomySelector.gtmpl"),
    @ComponentConfig(type = UISpaceCategoriesAddedList.class, id = "uiSpaceCategoriesAddedList", template = "war:groovy/social/webui/space/UISpaceCategoriesAddedList.gtmpl", events = {
        @EventConfig(listeners = UISpaceCategoriesAddedList.DeleteActionListener.class, confirm = "UICategoriesAddedList.msg.confirm-delete") }),
    @ComponentConfig(type = UIBreadcumbs.class, id = "BreadcumbOneTaxonomy", template = "system:/groovy/webui/core/UIBreadcumbs.gtmpl", events = {
        @EventConfig(listeners = { UIBreadcumbs.SelectPathActionListener.class }) }) })
...
public class UISpaceCategory extends UIContainer implements UISelectable {
  ...
  public void initOneTaxonomySelector() throws Exception {
    UIOneTaxonomySelector uiOneTaxonomySelector = addChild(UIOneTaxonomySelector.class,
                                                           "uiOneTaxonomySelector",
                                                           "uiOneTaxonomySelector");
    uiSpaceCategoriesAddedList = addChild(UISpaceCategoriesAddedList.class,
                                          "uiSpaceCategoriesAddedList",
                                          "uiSpaceCategoriesAddedList");
    NodeHierarchyCreator nodeHierarchyCreator = getApplicationComponent(NodeHierarchyCreator.class);
    String rootTreePath = nodeHierarchyCreator.getJcrPath(BasePath.TAXONOMIES_TREE_STORAGE_PATH);
    Session session = WCMCoreUtils.getUserSessionProvider().getSession(getWorkspaceName(), getRepository());
