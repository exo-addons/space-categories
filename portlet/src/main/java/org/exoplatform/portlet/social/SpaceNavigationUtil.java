...
public class SpaceNavigationUtil {
  ...
  private static TaxonomyService getTaxonomyService() {
    TaxonomyService taxonomyService = (TaxonomyService) ExoContainerContext.getCurrentContainer()
                                                                           .getComponentInstanceOfType(TaxonomyService.class);
  ...
  private static List<SpaceNavigationBean> getSpaces(Node categoryNode) {
    ...
    try {
      ...
      Query query = manager.createQuery("SELECT * FROM " + symlinkNodeType + " WHERE jcr:path LIKE '" + categoryNode.getPath()
          + "/%'", Query.SQL);
      ...
      while (nodes.hasNext()) {
        ...
        if (space != null) {
          ...
          SpaceNavigationBean spaceBean = new SpaceNavigationBean(space.getPrettyName(),
